...
public class GetLocationsHandler : IQueryHandler<GetLocationsQuery, PagedResult<LocationListItemDto>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            conditions.Add("name LIKE '%" + query.Search + "%'");
            parameters.Add("Search", query.Search);
        }

        if (query.PageSize > 0)
        {
            conditions.Add("pageSize=" + query.PageSize);
            parameters.Add("PageSize", query.PageSize);
        }

        if (query.Page > 0)
        {
            conditions.Add("page=" + query.Page);
        ...
        return new PagedResult<LocationListItemDto>(locationArr, query.Page, query.PageSize, pageCount, locationArr.First().TotalCount);
    }
}